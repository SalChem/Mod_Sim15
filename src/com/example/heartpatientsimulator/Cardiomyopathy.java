...
public class Cardiomyopathy extends Activity {
	...
	private void Connect(){
		...
		messageButton.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				startActivity(new Intent(Cardiomyopathy.this, Bluetooth.class));
			}
		});
	
		inputPath = "Cardiomyopathy Lead i.txt";
		InputStream input;
    ...
    leadi.getStyle().color = Color.GREEN;
	 ...
	 graphView = new LineGraphView(
		...
		graphView.setViewPort(0, 1600);
	    graphView.setManualYAxisBounds(.52, -.41);
			...
			public void run() {
				...
				if((n < signalDataArr.length) && (n + PULSE_WIDTH < signalDataArr.length)){
					for (int k = 0; k < PULSE_WIDTH; k = k + 1){
						...
						leadi.appendData(new GraphViewData(XValue, YValue), true,2300);
